    ...
    try {
        switch (obj.type) {
            ...
            case "aux":
                ...
                function drawaux(index, sing = true, place = -1, max = 1) {
                    var a, b, c;
                    a = index === -1 ? 1 : place === -1 ? index : place;
                    b = index === -1 ? "#000000" : cols[index];
                    c = sing ? 1 : max;
                    slice(a, b, false, atx, c, aucx, aucy, aurad)
                }
                if (obj.sing) {
                    drawaux(obj.index);
                } else {
                    for (let i = 0; i < obj.index.length; i++) {
                        drawaux(obj.index[i], false, i, obj.index.length);
                    }
                }
...
function lsd(base = -1) {
    ...
    while (!sorted) {
        ...
        for (i = 0; i < max; i++) {
            ...
            buckets[b].push(i);
            detog();
            if (!sorted || i === 0) aux(buckets[b], false, "Bucket");
        }
        expo *= base;
        var index = 0;
        var tc = 0;
